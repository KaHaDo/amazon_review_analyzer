...
def compute_coherence_values(vectorizer, X, dictionary, texts, limit, start=2, step=3):
    ...
    for num_topics in range(start, limit, step):
        ...
        coherencemodel = CoherenceModel(model=None, topics=topics, texts=texts, dictionary=dictionary, coherence='c_v',processes=-1)
...
if __name__ == '__main__':
    ...
    optimal_k = x[best_score_index]
    optimal_lda_model = model_list[best_score_index]

    print(f'\nOptimal number of topics is k={optimal_k} with a Coherence Score of {max(coherence_values):.4f}')
